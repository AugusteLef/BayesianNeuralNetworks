...
class BayesianLayer(torch.nn.Module):
    ...
    def forward(self, inputs):
        # TODO: enter your code here
        # cf. github/emptiyaz
        e_w = torch.nomral(mean=torch.zeros_like(self.weight_mu), std=1.0)
        w = self.weight_mu + F.softplus(self.weight_logsigma) * e_w

        if self.use_bias:
            # TODO: enter your code here
            # cf. github/emptiyaz
            e_b = torch.normal(mean=torch.zeros_like(self.bias_mu), std=1.0)
    ...
    def _kl_divergence(self, mu, logsigma):
        ...
        logsigma0 = torch.ones_like(logsigma) * self.prior_sigma

        kl = self._kl_gaussian(p_mu=mu, p_sigma=logsigma, q_mu=mu0, q_sigma=logsigma0)
        return kl
